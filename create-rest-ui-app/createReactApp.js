...
const program = new commander.Command(packageJson.name)
  ...
  .option(
    ...
    'use a non-standard version of rest-ui-scripts'
  )
  .allowUnknownOption()
  .on('--help', () => {
    ...
    console.log(
      `      - a custom fork published on npm: ${chalk.green('my-rest-ui-scripts')}`
    );
    console.log(
      `      - a .tgz archive: ${chalk.green('https://mysite.com/my-rest-ui-scripts-0.8.2.tgz')}`
    );
    console.log(
    ...
    console.log(
      `      ${chalk.cyan('https://github.com/RestUI/create-rest-ui-app/issues/new')}`
    );
    console.log();
  })
  .parse(process.argv);
...
function createApp(name, verbose, version, template) {
  ...
  console.log(`Creating a new Rest UI app in ${chalk.green(root)}.`);
  console.log();
...
function run(root, appName, version, verbose, originalDirectory, template) {
  ...
  getPackageName(packageToInstall)
    ...
    .then(packageName => {
      ...
      // Since rest-ui-scripts has been installed with --save
...
function getInstallPackage(version) {
  let packageToInstall = 'rest-ui-scripts';
...
function getPackageName(installPackage) {
  if (installPackage.indexOf('.tgz') > -1) {
    return getTemporaryDirectory()
      ...
      .catch(err => {
        // The package name could be with or without semver version, e.g. rest-ui-scripts-0.2.0-alpha.1.tgz
    ...
    // git+ssh://github.com/mycompany/rest-ui-scripts.git#v1.2.3
    ...
    console.error(
      chalk.red(
        'You are running Node %s.\n' +
          'Create Rest UI App requires Node %s or higher. \n' +
...
function checkAppName(appName) {
  ...
  const devDependencies = ['rest-ui-scripts'];
